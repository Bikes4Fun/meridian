...
class DementiaTVApp(App):
    ...
    def build(self):
        ...
        Window.size = (self.display_settings.window_width, self.display_settings.window_height)
        ...
        # Create factories
        self.screen_factory = ScreenFactory(self.services, self.screen_manager, display_settings=self.display_settings)
        self.widget_factory = WidgetFactory(self.services, display_settings=self.display_settings)
        ...
        self.screen_manager.current = "family" #TODO: make current whatever page we are working on
    ...
    def update_clock(self):
        ...
        if hasattr(self, 'home_screen'):
            ...
            if clock_widget:
                ...
                clock_widget.day_label.text = self.services['time_service'].get_dayof_week().upper()
                ...
                clock_widget.date_label.text = self.services['time_service'].get_month_day()
                if hasattr(clock_widget, 'year_label'):
                    clock_widget.year_label.text = self.services['time_service'].get_year()
    ...
    def _load_medications(self):
        ...
        if self.services.get('medication_service') and hasattr(self, 'home_screen'):
            ...
            if result.success:
                ...
                sorted_times = sorted(time_groups.keys(), key=lambda t: group_times.get(t, '23:59:59'))
                ...
                if prn_meds:
                    ...
                    for med in prn_meds:
                        last_taken = f"Last: {med['last_taken']}" if med['last_taken'] else "Not taken today"
                        meds_text.append(f"  • {med['name']}: {last_taken}")
                
                # Find medication widget in nested structure
                self._find_and_update_widget(self.home_screen, 'medication_content', 
                                           "\n".join(meds_text) if meds_text else "No medications")
            else:
                # Find medication widget and show error
                self._find_and_update_widget(self.home_screen, 'medication_content', "Error loading medications")
    ...
    def _load_events(self):
        ...
        if self.services.get('calendar_service') and hasattr(self, 'home_screen'):
            ...
            logger.debug(f"Loading events for today: {today.strftime('%Y-%m-%d')} (day={today.day})")
            result = self.services['calendar_service'].get_events_for_date(today.strftime("%Y-%m-%d"))
            
            logger.debug(f"Events query result: success={result.success}, data_count={len(result.data) if result.data else 0}")
            if result.data:
            ...
            if result.success and result.data:
                ...
                self._find_and_update_widget(self.home_screen, 'events_content', "\n".join(events_text))
            else:
                # Find events widget and show no events
                logger.debug(f"No events found for today, showing 'No events today'")
                self._find_and_update_widget(self.home_screen, 'events_content', "No events today")
    ...
    def update_time(self, dt=1):
        ...
        if hasattr(self, 'home_screen'):
            clock_widget = self._find_widget_by_attribute(self.home_screen, 'time_label')
            if clock_widget:
                clock_widget.time_label.text = self.services['time_service'].get_time()
                ...
                if current_time_of_day != self._last_time_of_day:
                    ...
                    if hasattr(clock_widget, 'time_of_day_icon'):
                        icon_path = self.widget_factory._get_time_of_day_icon(current_time_of_day)
                        clock_widget.time_of_day_icon.source = icon_path
...
class AppFactory:
    ...
    def create_application(self, user_id=None) -> DementiaTVApp:
        ...
        try:
            ...
            session = None
        display_settings = get_display_settings(server_url, user_id=user_id, session=session)
...
def create_app(config_manager: Optional[ConfigManager] = None, user_id=None) -> DementiaTVApp:
