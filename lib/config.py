...
def get_database_path() -> str:
    ...
    default_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "server", "database_management", "dementia_tv.db")
...
def find_available_port(host: str, start_port: int, max_tries: int = 20) -> int:
    ...
    for offset in range(max_tries):
        ...
        try:
            ...
            continue
    raise RuntimeError("No available port in range %s..%s" % (start_port, start_port + max_tries - 1))
