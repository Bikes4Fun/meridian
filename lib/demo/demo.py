    ...
    with db_manager.get_connection() as conn:
        ...
        for med in medications:
            ...
            for group_name in groups:
                # Get group ID
                cursor.execute("SELECT id FROM medication_groups WHERE name = ?", (group_name,))
...
def load_demo_settings_from_json_into_db(db_path, user_id):
    ...
    try:
        ...
        from lib.display.user_settings import DisplaySettings  # pyright: ignore[reportMissingImports]
    default_settings = DisplaySettings.default()  # single source: demo/demo_data/default_display_settings.json
...
def demo_main(user_id, db_path=None):
    ...
    try:        
        ...
        load_location_checkins_data(db_path, user_id = user_id)  # there should potentially be data from the webapp checkin
