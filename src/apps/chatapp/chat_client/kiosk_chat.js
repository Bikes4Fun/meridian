/**
 * Kiosk chat client – direct 1:1 chat. Requires sendbird_user_id in URL (set when kiosk user clicks a contact).
 * Uses session: GET /api/chat/config, POST /api/chat/token. No contact picker – recipient comes from URL.
 */
(function () {
    'use strict';
    ...
    function getRecipientFromUrl() {
        ...
        if (sb) return { sendbird_user_id: sb, name: name || sb };
        return null;
    }
    ...
    function run() {
        var recipient = getRecipientFromUrl();
        if (!recipient) {
            setStatus('Missing recipient. Close and select a contact from the kiosk.', 'error');
            return;
        }
        ...
        Promise.all([loadConfig(), loadToken()])
            .then(function (results) {
                ...
                if (loggedInEl) loggedInEl.textContent = 'Chat with ' + (recipient.name || recipient.sendbird_user_id) + '. ';
                setStatus('Initializing Sendbird…', 'info');
                return initSendbird(config.app_id, sendbirdUserId, tokenData.session_token, recipient.sendbird_user_id);
            })
            .catch(function (err) {
    ...
    function initSendbird(appId, sendbirdUserId, sessionToken, recipientSendbirdUserId) {
        ...
        return import(sdkUrl)
            ...
            .then(function () {
                setStatus('Opening conversation…', 'success');
            ...
            .then(function (list) {
                ...
                setStatus('You can send messages below.', 'success');
