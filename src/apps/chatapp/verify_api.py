...
def _fmt(v):
    return json.dumps(v, indent=2) if isinstance(v, dict) else json.dumps(v, indent=2) if isinstance(v, list) else str(v)
...
def verify_api(api_url, logger=None):
    ...
    # 1. login
    query = "POST %s/api/login\n  body: %s" % (base, json.dumps({"user_id": "fm_care_001", "family_circle_id": "F00000"}))
    try:
        r = session.post(base + "/api/login", json={"user_id": "fm_care_001", "family_circle_id": "F00000"}, timeout=5)
    ...
    if app_id and api_token and sender:
        ...
        try:
            r = requests.post(sb_base + "/group_channels", headers={"Api-Token": api_token, "Content-Type": "application/json"}, json=body, timeout=10)
            resp = r.json() if r.headers.get("content-type", "").startswith("application/json") else r.text
        ...
        if channel_url:
            ...
            try:
                r2 = requests.post(sb_base + "/group_channels/" + channel_url + "/messages", headers={"Api-Token": api_token, "Content-Type": "application/json"}, json=body, timeout=10)
                resp2 = r2.json() if r2.headers.get("content-type", "").startswith("application/json") else r2.text
            ...
            try:
                ...
                resp3 = r3.json() if r3.headers.get("content-type", "").startswith("application/json") else r3.text
                steps.append(("list messages", query, r3.status_code, resp3))
            except Exception as e:
                steps.append(("list messages", query, "ERR", str(e)))
    else:
        steps.append(("Sendbird", "(skipped)", "SKIP", "SENDBIRD_APP_ID/API_TOKEN or token missing"))
