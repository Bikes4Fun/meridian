...
def _post(
    ...
    """POST URL with JSON body. Returns (ok, data or dict, error)."""
    try:
        import requests
    except ImportError:
        return False, None, "requests not installed"
    try:
        client = session if session else requests
        r = client.post(url, json=json_body or {}, timeout=timeout, headers=headers or {})
        r.raise_for_status()
        j = r.json()
        if "error" in j:
            return False, None, j["error"]
...
class RemoteVideoService:
    ...
    def get_participant_token(self) -> Any:
        """POST to get Dyte participant auth token. Returns ServiceResult with data.authToken or error."""
        ok, data, err = _post(
        ...
        if not ok:
            return ServiceResult.error_result(err or "video participant-token request failed")
        token = data.get("authToken") if isinstance(data, dict) else None
        if not token:
            return ServiceResult.error_result("No authToken in response")
        return ServiceResult.success_result({"authToken": token})
