...
def populated_test_db(test_db_manager, sample_contacts_data):
    ...
    r = db.execute_query(
        "SELECT id FROM medication_times WHERE family_circle_id = ? ORDER BY name",
        (FAMILY_CIRCLE_ID,),
    )
    ...
    med_ids = (
        [row["id"] for row in med_r.data] if med_r.success and med_r.data else [1, 2]
    )
    db.execute_update(
        "INSERT OR IGNORE INTO medication_to_time (medication_id, group_id) VALUES (?, ?)",
        (med_ids[0], morning_id),
    )
    db.execute_update(
        "INSERT OR IGNORE INTO medication_to_time (medication_id, group_id) VALUES (?, ?)",
        (med_ids[1], afternoon_id),
    )
